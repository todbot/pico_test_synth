...
import touchio
import audiomixer, audiobusio
import synthio
import displayio, i2cdisplaybus
import adafruit_displayio_ssd1306

# values you may want to edit to taste
...
class PicoTestSynthHardware():
    ...
    def __init__(self, pull_type=digitalio.Pull.UP):
        """Create a PicoTestSynthHardware object that instantiates the
           CircuitPython hardware objects for all the hardware """
        ...
        # LED on pico, single button & two knobs on PCB
        ...
        # do audio setup after display, so we have minimal audible glitches
        ...
        self.mixer = audiomixer.Mixer(sample_rate=SAMPLE_RATE,
                                      voice_count=1, channel_count=NUM_CHANNELS,
                                     bits_per_sample=16, samples_signed=True,
                                     buffer_size=MIXER_BUFFER_SIZE)
        self.synth = synthio.Synthesizer(sample_rate=SAMPLE_RATE, channel_count=NUM_CHANNELS)
        self.audio.play(self.mixer)
        self.mixer.voice[0].play(self.synth)

    def check_key(self):
        """Check the single physical button, return keypad-like Events"""
    ...
    def read_pots(self):
        """Read the knobs, filter out their noise, return as int tuple"""
        filt = 0.5
